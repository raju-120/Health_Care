...
export const google = async(req,res,next) =>{
    try{
        ...
        if(user){
            ...
            console.log('coming from google user',rest);
...
export const doctorSignIn = async(req, res, next) =>{
    ...
    try{
        const validUser = await Doctor.findOne({email: req.body.email})
        if(!validUser) return next(errorHandler(404,'Doctors id is not found!'));
